...
program.option('--no-custom-user-agent', 'Boolean when specified does not modify useragent');
...
app.on('ready', function handleReady () {
  ...
  if (program.customUserAgent) {
    ...
    customParams.userAgent = 'Electron ' + process.versions.electron + ' (Node ' + process.versions.node + ')';
  }
  browserWindow.loadURL(program.url, customParams);
});
